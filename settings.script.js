document.addEventListener('DOMContentLoaded', () => {
    ...
    const addHonourBtn = document.getElementById('addHonourBtn');
    ...
    if (saveSettingsBtn) {
        saveSettingsBtn.addEventListener('click', () => {
            showModal("Configuration saved successfully. All general settings and elite archive parameters have been updated.");
        });
    }

    if (addHonourBtn) {
        addHonourBtn.addEventListener('click', () => {
            showModal("Elite Archive Editor: Season winner and MVP registration records are ready for addition.");
        });
    }

    if (clearCacheBtn) {
        clearCacheBtn.addEventListener('click', () => {
            showModal("Local storage cache cleared successfully. All temporary session data has been reset.");
        });
    }

    if (exportDataBtn) {
        exportDataBtn.addEventListener('click', () => {
            showModal("League Archive Export: Tournament JSON package generated and ready for download.");
        });
    }
